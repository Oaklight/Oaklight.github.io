const I18N = {
  en: {
    ...
    advisor: 'Advisor',
    side_interest: 'Outside research, I enjoy building self-hosted infrastructure, including a multi-server private cloud with GeoDNS-optimized access to distributed services.',
    tag_agents: 'AI Agents',
    tag_tool: 'Tool Orchestration',
    tag_multiagent: 'Multi-Agent Collaboration',
    tag_llm: 'LLM Systems',
    research_focus: 'My research focuses on making LLM systems more interoperable, tool-using, and reliable in practice. I build abstractions for unified access across heterogeneous LLM API standards and tool ecosystems, and I study constrained software design and evaluation methods that improve correctness, deployability, and real-world usability.',
    ...
    proj_comply: 'Multi-format LLM API compliance testing CLI',
    ...
    proj_veilrender: 'Headless browser rendering API with stealth capabilities',
    ...
    proj_tinyleaf: 'Tiny, local-first, browser-based TeX editor with PDF preview and Git tools',
    proj_asr2clip: 'CLI tool to convert speech to clipboard text',
    cv_download: 'download (PDF)',
    pub_zerodep: 'Stdlib or Third-Party? Empirical Performance and Correctness of LLM-Assisted Zero-Dependency Python Libraries',
    pub_rosetta: 'LLM-Rosetta: A Hub-and-Spoke Intermediate Representation for Cross-Provider LLM API Translation',
    pub_unified_tool: 'Unified Tool Integration for LLMs: A Protocol-Agnostic Approach to Function Calling',
  ...
  zh: {
    ...
    university: '芝加哥大学',
    advisor: '导师',
    side_interest: '在研究之外，我也喜欢搭建自托管基础设施，包括一个由多台服务器组成、并通过 GeoDNS 优化分布式服务访问的个人私有云。',
    tag_agents: 'AI 智能体',
    tag_tool: '工具编排',
    tag_multiagent: '多智能体协作',
    tag_llm: 'LLM 系统',
    research_focus: '我的研究聚焦于让 LLM 系统在真实场景中更具互操作性、工具使用能力与可靠性。我构建面向异构 LLM API 标准与工具生态的统一访问抽象，并研究受约束的软件设计与评测方法，以提升系统的正确性、可部署性与实际可用性。',
    cat_selected: '精选项目',
    cat_notable: '值得一提的工具',
    ...
    proj_tinyleaf: '本地优先的轻量级浏览器 TeX 编辑器，支持 PDF 预览和 Git 工具',
    proj_asr2clip: '语音转剪贴板文本 CLI 工具',
    cv_download: '下载简历 (PDF)',
    pub_zerodep: 'Stdlib or Third-Party? LLM 辅助零依赖 Python 库的性能与正确性实证研究',
    pub_rosetta: 'LLM-Rosetta：面向跨提供商 LLM API 翻译的中心辐射型中间表示',
    pub_unified_tool: 'LLM 统一工具集成：协议无关的函数调用方法',
